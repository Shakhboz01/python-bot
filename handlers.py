...
def main_menu():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🔍 Option 1"), KeyboardButton(text="📞 Contact Support")],
...
async def ask_full_name(message: Message):
    ...
    if chat_id in user_data and "full_name" not in user_data[chat_id]:
        ...
        await message.answer("Great! Now, please share your phone number.")
        ...
        if not PHONE_REGEX.match(message.text.strip()):
            ...
            return
        add_user(chat_id, user_data[chat_id]["full_name"], message.text)
